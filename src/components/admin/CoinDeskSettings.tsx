...
const CoinDeskSettings: React.FC = () => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('coindesk_enabled, coindesk_api_key, coindesk_last_run, coindesk_rate_limited')
      ...
      setSettings({
        enabled: data.coindesk_enabled || false,
        api_key: data.coindesk_api_key || '',
        last_run: data.coindesk_last_run,
        rate_limited: data.coindesk_rate_limited || false
      });
    } catch (err) {
      console.error('Error fetching CoinDesk settings:', err);
      setError(err instanceof Error ? err.message : 'Failed to load CoinDesk settings');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveSettings = async () => {
    try {
      ...
      await aggregationService.updateCoindeskSettings({
      ...
      console.error('Error saving CoinDesk settings:', err);
  ...
  const runCoindeskAggregation = async () => {
    try {
      setRunningAggregation(true);
      setError(null);
      setAggregationResult(null);

      const result = await aggregationService.triggerCoindeskAggregation();
      setAggregationResult(result);
      
      // Refresh settings after running
      await fetchSettings();
    } catch (err) {
      console.error('Error running CoinDesk aggregation:', err);
      setError(err instanceof Error ? err.message : 'Failed to run CoinDesk aggregation');
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-medium text-white">CoinDesk API Configuration</h2>
        <button
      ...
      {saveSuccess && (
        ...
        >
          ...
          CoinDesk settings saved successfully
        </motion.div>
      )}
      ...
      <div className="bg-gray-800/50 rounded-lg border border-gray-700/50 overflow-hidden">
        <div className="p-6 border-b border-gray-700/50">
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-300">Enable CoinDesk API</span>
              <button 
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-400">CoinDesk API Key</label>
              ...
              <p className="text-xs text-gray-500">
                Required to use the CoinDesk API. You can get a key by signing up at{' '}
                <a href="https://api.coindesk.com/sign-up" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">
                  api.coindesk.com
                </a>
              </p>
            </div>
              ...
              <div className="p-3 bg-gray-800/80 rounded-lg">
                ...
                {settings.rate_limited && (
                  <div className="flex items-center text-yellow-400 text-sm">
                    ...
                    API currently rate limited. Try again in 24 hours.
                  </div>
                )}
              </div>
            )}
            ...
            <div className="p-3 bg-blue-900/20 border border-blue-500/30 rounded-lg text-sm text-blue-300">
              <div className="flex items-start">
                ...
                <div>
                  <p className="font-medium mb-1">About the CoinDesk API</p>
                  <ul className="list-disc pl-5 space-y-1 text-gray-300">
                    <li>The CoinDesk API has strict rate limits for free accounts</li>
                    <li>Rate limits are typically 10 requests per day</li>
                    <li>If rate limited, the system will automatically disable requests for 24 hours</li>
                    <li>Enable this option only if you have a valid API key</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center pt-4">
              ...
              <button
                onClick={runCoindeskAggregation}
          ...
          <div className="p-6 bg-gray-900/30 border-t border-gray-700/50">
            ...
            <div className="bg-gray-800/70 p-4 rounded-lg">
              ...
              {aggregationResult.count > 0 ? (
                <p className="text-green-400">Successfully added {aggregationResult.count} new articles from CoinDesk</p>
              ) : (
                <p className="text-yellow-400">No new articles were added from CoinDesk</p>
              )}
              ...
              {aggregationResult.status === 'skipped' && (
                <div className="mt-2 bg-yellow-900/30 p-3 rounded-lg text-yellow-300">
                  ...
                  <p className="text-sm mt-1">{aggregationResult.reason || 'CoinDesk API is disabled in system settings'}</p>
                </div>
              )}
